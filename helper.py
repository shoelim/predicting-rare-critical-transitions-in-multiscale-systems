...
def select_and_plot_results(index_data, n_ens, trainlen, valid, future, trainbeg, dtau, option):
    ...
    if option == 1:
      ...
      osol = pd.read_csv('Baseline_Ex'+'{0}'.format(index_data)+'_'+'{0}'.format(trainlen)+'-'+'{0}'.format(valid)+'_'+'{0}'.format(n_ens)+'ens_fin.csv',header=None)
    else:
    ...
    #ax2.text(0.1, 0.96,'(b)', fontsize=12, verticalalignment='top')
    plt.xlabel('$t$')
    plt.ylabel('$x$')
    if option == 0:
      plt.savefig('Baseline_Ex'+'{0}'.format(index_data)+'_'+'{0}'.format(trainlen)+'_'+'{0}'.format(valid)+'_res.png')
    #plt.grid(False)
    #plt.show()
    ...
    if option == 1:
      ...
      #=====================================================
      ax3.text(0.2, 0.7,'$N_{eff}$='+'{0}'.format(N_eff), fontsize=12) 
      plt.xlabel('$t$')
      plt.ylabel('$x$')
      ...
      #ax4.text(0.1, 0.96,'(c)', fontsize=12, verticalalignment='top')
      plt.xlabel('$t$')
      plt.ylabel('$E_{out}$')
      ...
      #ax5.text(0.1, 0.96,'(d)', fontsize=12, verticalalignment='top')
      plt.xlabel('$t$')
